#!/usr/bin/env bash
# run_all.sh — Run all git-lab exercise tests in order
#
# Usage:
#   bash tests/run_all.sh              # run all tests
#   bash tests/run_all.sh --skip 06   # skip exercise 6 (stash)
#
# Prerequisites: git-lab VM must be running (qlab run git-lab)
...
echo ""
echo "${BOLD}=========================================${RESET}"
echo "${BOLD}  git-lab — Automated Test Suite${RESET}"
echo "${BOLD}=========================================${RESET}"
echo ""
...
log_info "Checking git workspace..."
assert "Git workspace exists" ssh_vm "test -d ~/workspace/.git"
